...
import { AI_RESPONSES } from '../assets/mockData';

/* ─────────────── shared card ─────────────── */
const Card = ({ children, style = {} }) => {
  const { isNight: N } = useApp();
  return (
    <div style={{ background: N ? 'rgba(8,20,52,0.9)' : '#fff', border: `1px solid ${N ? 'rgba(100,180,255,0.1)' : 'rgba(26,123,202,0.08)'}`, borderRadius: 16, padding: '20px 22px', boxShadow: N ? '0 4px 20px rgba(0,0,0,0.28)' : '0 4px 20px rgba(26,90,138,0.07)', ...style }}>
      {children}
    </div>
  );
};

const LoadingSpinner = ({ N }) => (
  <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--t-muted)', fontSize: 14 }}>
    <div style={{ textAlign: 'center' }}>
      <div style={{ width: 36, height: 36, border: `3px solid ${N ? 'rgba(77,168,240,0.2)' : '#e0f0fb'}`, borderTopColor: N ? '#4da8f0' : '#1a7bca', borderRadius: '50%', animation: 'spin .7s linear infinite', margin: '0 auto 12px' }} />
      Chargement...
    </div>
  </div>
);
...
export const VaccinsPage = () => {
  const { isNight: N, selectedChild, setSelectedChild, appChildren, dataLoaded } = useApp();
  const [filter, setFilter] = useState('all');

  if (!dataLoaded) return <LoadingSpinner N={N} />;
  if (appChildren.length === 0) return (
    <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <p style={{ color: 'var(--t-muted)', fontSize: 14 }}>Aucun enfant enregistré.</p>
    </div>
  );

  const child = appChildren[selectedChild] || appChildren[0];
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '32px 36px 40px' }}>
      ...
      <Card>
        {filtered.length === 0 ? (
          <p style={{ textAlign: 'center', color: 'var(--t-muted)', fontSize: 13, padding: '16px 0' }}>Aucun vaccin dans cette catégorie.</p>
        ) : (
          filtered.map((v, i, arr) => (
            <div key={v.id || i} style={{ display: 'flex', gap: 14, padding: '14px 0', borderBottom: i < arr.length - 1 ? `0.5px solid ${N ? 'rgba(100,180,255,0.08)' : 'rgba(26,123,202,0.07)'}` : 'none' }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    ...
                    {v.status === 'done' && v.doctor && <p style={{ fontSize: 11, color: 'var(--t-muted)' }}>👨‍⚕️ {v.doctor}{v.hospital ? ` · 🏥 ${v.hospital}` : ''}</p>}
...
export const CalendarPage = () => {
  const { isNight: N, selectedChild, appChildren, dataLoaded } = useApp();
  ...
  const vacDays = child.vaccines.filter(v => v.status === 'upcoming').map(v => {
    if (v.scheduledDate) {
      const d = new Date(v.scheduledDate + 'T00:00:00');
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <Card>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
            ...
            <span style={{ fontFamily: 'var(--font-main)', fontWeight: 800, fontSize: 16, color: N ? '#e2f0ff' : '#1a3a5c' }}>{MONTHS[month]} {year}</span>
        ...
        <Card>
          ...
          {child.vaccines.length === 0 ? (
            <p style={{ fontSize: 13, color: 'var(--t-muted)', textAlign: 'center', padding: '12px 0' }}>Aucun vaccin enregistré.</p>
          ) : (
            child.vaccines.map((v, i, arr) => (
              <div key={v.id || i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: i < arr.length - 1 ? `0.5px solid ${N ? 'rgba(100,180,255,0.08)' : 'rgba(26,123,202,0.07)'}` : 'none' }}>
...
/* ════════════════ CHAT IA — fallback local ════════════════ */
const SYSTEM_PROMPT = `Tu es VacciBot, l'assistant IA expert en vaccination intégré à VacciSmart. Tu aides les parents et les médecins en Afrique francophone.`;
...
export const ChatPage = () => {
  const { isNight: N, user, selectedChild, appChildren } = useApp();
  const child = appChildren[selectedChild] || appChildren[0];
  const userInitial = (user?.firstName || user?.name || 'U')[0].toUpperCase();

  const welcomeText = child
    ? `👋 Bonjour ${user?.firstName || ''} ! Je suis **VacciBot**, votre assistant vaccinal.\n\nJe connais le dossier de **${child.firstName}** et je peux répondre à toutes vos questions sur les vaccins, les effets secondaires et le calendrier vaccinal.\n\nComment puis-je vous aider ?`
    : `👋 Bonjour ! Je suis **VacciBot**, votre assistant vaccinal IA.\n\nComment puis-je vous aider ?`;

  const [msgs,    setMsgs]    = useState([{ role: 'assistant', text: welcomeText }]);
  ...
  const callAPI = async (userMsg) => {
    ...
    const key = Object.keys(AI_RESPONSES).find(k => msgLower.includes(k));
  ...
  const send = async (textOverride) => {
    ...
    try {
      ...
      setMsgs(m => [...m, { role: 'assistant', text: '⚠️ Une erreur est survenue. Réessayez.', error: true }]);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden' }}>
      ...
      <div style={{ padding: '20px 32px 16px', borderBottom: `1px solid ${N ? 'rgba(100,180,255,0.08)' : 'rgba(26,123,202,0.08)'}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: '#22c55e', fontWeight: 600 }}>● En ligne · Base de connaissances médicale</p>
          </div>
        </div>
        {child && (
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px 32px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        {msgs.map((m, i) => (
          <div key={i} className="fade-in" style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start', alignItems: 'flex-end', gap: 10 }}>
            ...
            <div style={{ maxWidth: '68%', padding: '13px 17px', lineHeight: 1.75, fontSize: 14, borderRadius: m.role === 'user' ? '18px 18px 4px 18px' : '4px 18px 18px 18px', background: m.role === 'user' ? 'linear-gradient(135deg,#1a7bca,#2196f3)' : (N ? 'rgba(10,24,60,0.95)' : '#fff'), color: m.role === 'user' ? '#fff' : (N ? '#e2f0ff' : '#1a3a5c'), border: m.role === 'assistant' ? `1px solid ${N ? 'rgba(100,180,255,0.14)' : 'rgba(26,123,202,0.09)'}` : 'none', boxShadow: m.role === 'user' ? '0 4px 14px rgba(33,150,243,0.35)' : (N ? '0 2px 14px rgba(0,0,0,0.28)' : '0 2px 14px rgba(26,90,138,0.07)'), whiteSpace: 'pre-wrap', opacity: m.error ? 0.75 : 1 }}>
              {formatText(m.text)}
            </div>
            {m.role === 'user' && (
      ...
      <div style={{ padding: '12px 32px 20px', display: 'flex', gap: 10, alignItems: 'flex-end', flexShrink: 0, borderTop: `1px solid ${N ? 'rgba(100,180,255,0.07)' : 'rgba(26,123,202,0.07)'}` }}>
        ...
        />
        <button onClick={() => send()} disabled={!input.trim() || loading} style={{ width: 46, height: 46, borderRadius: '50%', border: 'none', background: input.trim() && !loading ? 'linear-gradient(135deg,#1a7bca,#2196f3)' : (N ? 'rgba(255,255,255,0.07)' : '#e8f3fc'), color: input.trim() && !loading ? '#fff' : 'var(--t-muted)', fontSize: 20, cursor: input.trim() && !loading ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all .2s', boxShadow: input.trim() && !loading ? '0 4px 14px rgba(33,150,243,0.4)' : 'none' }}>
          {loading ? <div style={{ width: 18, height: 18, border: '2.5px solid rgba(255,255,255,0.3)', borderTopColor: input.trim() ? '#fff' : 'var(--t-muted)', borderRadius: '50%', animation: 'spin .7s linear infinite' }} /> : '➤'}
        </button>
      </div>
    </div>
  );
...
export const NotificationsPage = () => {
  const { isNight: N, reminders } = useApp();

  const HARDCODED = [
    { emoji: '🔔', title: 'Rappel vaccin ROR', desc: 'Demain à 10h00 · Hôpital Central', time: 'Il y a 1h', type: 'urgent' },
    { emoji: '✅', title: 'Vaccin DTP 1 confirmé', desc: '15 Nov 2025 · Dr. Kamga', time: 'Il y a 2 jours', type: 'success' },
    { emoji: '💉', title: 'Prochain vaccin dans 3 jours', desc: 'DTP 2 · 25 janv. 2026', time: 'Il y a 3 jours', type: 'info' },
    { emoji: '🤖', title: 'Recommandation VacciBot', desc: 'Calendrier vaccinal à jour recommandé', time: 'Il y a 5 jours', type: 'ai' },
    { emoji: '🏥', title: 'Rappel établissement', desc: 'Confirmer le prochain RDV', time: 'Il y a 6 jours', type: 'info' },
  ];

  const apiNotifs = reminders.map(r => ({
    emoji: r.sent ? '✅' : '🔔',
    title: `Rappel — ${r.child_name}`,
    desc: r.message || `Vaccination programmée · ${r.channel}`,
    time: new Date(r.send_at).toLocaleDateString('fr-FR'),
    type: r.sent ? 'success' : 'urgent',
  }));

  const notifs = apiNotifs.length > 0 ? apiNotifs : HARDCODED;

  const colors = {
    urgent:  { bg: N ? 'rgba(245,158,11,0.1)' : '#fffbeb',   border: N ? 'rgba(245,158,11,0.35)' : '#fde68a',   dot: '#f59e0b' },
    success: { bg: N ? 'rgba(34,197,94,0.08)' : '#f0fdf4',   border: N ? 'rgba(34,197,94,0.25)' : '#bbf7d0',    dot: '#22c55e' },
    info:    { bg: N ? 'rgba(77,168,240,0.08)' : '#f0f8ff',  border: N ? 'rgba(100,180,255,0.2)' : '#bfdbfe',   dot: '#4da8f0' },
    ai:      { bg: N ? 'rgba(139,92,246,0.08)' : '#f5f3ff',  border: N ? 'rgba(139,92,246,0.25)' : '#ddd6fe',   dot: '#8b5cf6' },
  };
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '32px 36px 40px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {notifs.map((n, i) => { const c = colors[n.type] || colors.info; return (
...
export const HospitalsPage = () => {
  const { isNight: N, hospitals, dataLoaded } = useApp();
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '32px 36px 40px' }}>
      <h1 style={{ fontFamily: 'var(--font-main)', fontWeight: 900, fontSize: 28, color: N ? '#e2f0ff' : '#1a3a5c', marginBottom: 6 }}>🏥 Hôpitaux & centres</h1>
      <p style={{ fontSize: 13, color: 'var(--t-muted)', marginBottom: 24 }}>Centres de vaccination disponibles</p>
      {!dataLoaded ? (
        <LoadingSpinner N={N} />
      ) : hospitals.length === 0 ? (
        <Card><p style={{ textAlign: 'center', color: 'var(--t-muted)', fontSize: 13, padding: '20px 0' }}>Aucun centre enregistré. Contactez l'administrateur.</p></Card>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          {hospitals.map((h, i) => (
            <Card key={h.id || i}>
              ...
              {h.phone && <p style={{ fontSize: 12, color: 'var(--t-muted)', marginBottom: 12 }}>📞 {h.phone}</p>}
              {h.vaccines?.length > 0 && (
              ...
              <button style={{ marginTop: 4, width: '100%', padding: '9px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg,#1a7bca,#2196f3)', color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-main)' }}>📞 Prendre RDV</button>
            </Card>
          ))}
        </div>
      )}
...
export const ProfilePage = () => {
  ...
  const Toggle = ({ on }) => (
    <div style={{ width: 44, height: 24, borderRadius: 12, background: on ? 'linear-gradient(135deg,#1a7bca,#2196f3)' : (N ? 'rgba(255,255,255,0.1)' : '#e0f0fb'), position: 'relative', cursor: 'pointer', transition: 'background .3s' }}>
      <div style={{ position: 'absolute', top: 3, left: on ? 23 : 3, width: 18, height: 18, borderRadius: '50%', background: '#fff', transition: 'left .25s', boxShadow: '0 1px 5px rgba(0,0,0,0.2)' }} />
    </div>
  );
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '32px 36px 40px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <div>
          ...
          <Card>
            ...
            {[['Email', user?.email], ['Rôle', role === 'parent' ? 'Parent / Tuteur' : 'Médecin'], ['Téléphone', user?.phone || '—'], ['Ville', user?.city || '—']].map(([k, v], i, arr) => (
        ...
        <div>
          <Card style={{ marginBottom: 16 }}>
            ...
            {[['Rappels vaccins J-1', true], ['Rappels mensuels', true], ['Alertes retards', true], ['Newsletter', false]].map(([l, on], i, arr) => (
              <div key={l} style={{ ...row, ...(i < arr.length - 1 ? div : {}) }}>
                <span style={{ fontSize: 12, color: N ? '#e2f0ff' : '#1a3a5c' }}>{l}</span>
                <Toggle on={on} />
              </div>
            ))}
          </Card>
...
/* ════════════════ DOCTOR UPCOMING ════════════════ */
const urgColor = (d, N) =>
  d <= 1 ? { bg: N ? 'rgba(239,68,68,0.12)' : '#fee2e2',   text: N ? '#f87171' : '#dc2626', border: N ? 'rgba(239,68,68,0.3)' : '#fca5a5' }
  : d <= 3 ? { bg: N ? 'rgba(245,158,11,0.1)' : '#fef3c7',  text: N ? '#fbbf24' : '#92400e', border: N ? 'rgba(245,158,11,0.3)' : '#fde68a' }
           : { bg: N ? 'rgba(77,168,240,0.1)' : '#e0f0fb',  text: N ? '#4da8f0' : '#1a7bca', border: N ? 'rgba(100,180,255,0.2)' : '#bfdbfe' };

export const DoctorUpcomingPage = () => {
  const { isNight: N, user, doctorPatients, dataLoaded } = useApp();
  const [search, setSearch] = useState('');
  const filtered = doctorPatients.filter(p =>
    p.name.toLowerCase().includes(search.toLowerCase()) ||
    p.vaccine.toLowerCase().includes(search.toLowerCase())
  );
    ...
    <div style={{ flex: 1, overflowY: 'auto', padding: '32px 36px 40px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--t-muted)' }}>{user?.name} · {user?.hospital_code || 'Hôpital'}</p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 24 }}>
        {[
          { l: 'Demain', n: doctorPatients.filter(p => p.daysLeft <= 1).length, c: '#ef4444' },
          { l: '≤ 3 jours', n: doctorPatients.filter(p => p.daysLeft <= 3).length, c: '#f59e0b' },
          { l: 'Total', n: doctorPatients.length, c: N ? '#4da8f0' : '#1a7bca' },
        ].map(s => (
      ...
      {!dataLoaded ? (
        ...
        <Card><p style={{ textAlign: 'center', color: 'var(--t-muted)', fontSize: 13, padding: '20px 0' }}>
          {search ? 'Aucun résultat pour cette recherche.' : 'Aucun patient avec vaccin à venir. Vérifiez les assignations dans l\'administration.'}
        </p></Card>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {filtered.map((p) => { const u = urgColor(p.daysLeft, N); return (
            <div key={p.id} style={{ background: N ? 'rgba(8,20,52,0.9)' : '#fff', border: `1px solid ${u.border}`, borderRadius: 16, padding: '16px 20px', boxShadow: N ? '0 4px 20px rgba(0,0,0,0.28)' : '0 4px 20px rgba(26,90,138,0.07)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 11, color: 'var(--t-muted)' }}>{p.age}{p.parent ? ` · Parent : ${p.parent}` : ''}</p>
                  </div>
                </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderRadius: 12, background: N ? 'rgba(255,255,255,0.04)' : '#f8fbff' }}>
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ fontFamily: 'var(--font-main)', fontWeight: 700, fontSize: 13, color: N ? '#e2f0ff' : '#1a3a5c', marginBottom: 1 }}>{p.vaccine}</p>
                  {p.phone && <p style={{ fontSize: 11, color: 'var(--t-muted)' }}>📞 {p.phone}</p>}
                </div>
...
export const DoctorRemindersPage = () => {
  const { isNight: N, doctorPatients, dataLoaded } = useApp();
  const notified = doctorPatients.filter(p => p.notified);
  const pending  = doctorPatients.filter(p => !p.notified);

  const PCard = ({ p, type }) => {
    const colors = {
      sent:    { bg: N ? 'rgba(34,197,94,0.06)' : '#f0fdf4',  border: N ? 'rgba(34,197,94,0.2)' : '#bbf7d0' },
      pending: { bg: N ? 'rgba(245,158,11,0.06)' : '#fffbeb', border: N ? 'rgba(245,158,11,0.22)' : '#fde68a' },
    };
    const c = colors[type];
    return (
      <div style={{ background: c.bg, border: `0.5px solid ${c.border}`, borderRadius: 14, padding: '14px 18px', marginBottom: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <p style={{ fontFamily: 'var(--font-main)', fontWeight: 700, fontSize: 13, color: N ? '#e2f0ff' : '#1a3a5c', marginBottom: 3 }}>{p.name}</p>
          <p style={{ fontSize: 11, color: 'var(--t-muted)', marginBottom: 2 }}>💉 {p.vaccine} · {p.date}</p>
          {p.phone && <p style={{ fontSize: 11, color: 'var(--t-muted)' }}>📞 {p.phone}</p>}
        </div>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 22 }}>{type === 'sent' ? '✅' : '⏰'}</div>
          <p style={{ fontSize: 10, color: type === 'sent' ? (N ? '#4ade80' : '#16a34a') : (N ? '#fbbf24' : '#92400e'), fontWeight: 800, marginTop: 2 }}>{type === 'sent' ? 'Envoyé' : 'Programmé'}</p>
        </div>
      </div>
    );
  };
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '32px 36px 40px' }}>
      ...
      <Card style={{ marginBottom: 20, background: N ? 'rgba(0,184,163,0.08)' : '#ecfdf5', border: `0.5px solid ${N ? 'rgba(0,229,196,0.2)' : '#a7f3d0'}`, display: 'flex', alignItems: 'center', gap: 16 }}>
        ...
        <div>
          <p style={{ fontFamily: 'var(--font-main)', fontWeight: 800, fontSize: 15, color: N ? '#00e5c4' : '#065f46', marginBottom: 3 }}>{notified.length} rappels envoyés</p>
          <p style={{ fontSize: 12, color: N ? '#00c4a0' : '#047857', lineHeight: 1.6 }}>Notifications push 24h avant chaque RDV · Automatique même si l'app est fermée</p>
        </div>
      </Card>
      {!dataLoaded ? (
        <LoadingSpinner N={N} />
      ) : doctorPatients.length === 0 ? (
        <Card><p style={{ textAlign: 'center', color: 'var(--t-muted)', fontSize: 13, padding: '20px 0' }}>Aucun patient assigné.</p></Card>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          <div>
            <p style={{ fontFamily: 'var(--font-main)', fontWeight: 800, fontSize: 13, color: N ? '#4ade80' : '#16a34a', marginBottom: 12 }}>✅ Envoyés ({notified.length})</p>
            {notified.length === 0 ? <p style={{ fontSize: 12, color: 'var(--t-muted)' }}>Aucun rappel envoyé.</p> : notified.map(p => <PCard key={p.id} p={p} type="sent" />)}
          </div>
          <div>
            <p style={{ fontFamily: 'var(--font-main)', fontWeight: 800, fontSize: 13, color: N ? '#fbbf24' : '#92400e', marginBottom: 12 }}>⏳ Programmés ({pending.length})</p>
            {pending.length === 0 ? <p style={{ fontSize: 12, color: 'var(--t-muted)' }}>Aucun rappel en attente.</p> : pending.map(p => <PCard key={p.id} p={p} type="pending" />)}
          </div>
        </div>
      )}
    </div>
