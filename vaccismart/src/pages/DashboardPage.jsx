...
const LoadingState = ({ N }) => (
  <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--t-muted)', fontSize: 14 }}>
    <div style={{ textAlign: 'center' }}>
      <div style={{ width: 40, height: 40, border: `3px solid ${N ? 'rgba(77,168,240,0.2)' : '#e0f0fb'}`, borderTopColor: N ? '#4da8f0' : '#1a7bca', borderRadius: '50%', animation: 'spin .7s linear infinite', margin: '0 auto 16px' }} />
      Chargement des données...
    </div>
  </div>
);

const EmptyState = ({ N, setPage }) => (
  <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40 }}>
    <div style={{ textAlign: 'center', maxWidth: 360 }}>
      <div style={{ fontSize: 64, marginBottom: 16 }}>👶</div>
      <h2 style={{ fontFamily: 'var(--font-main)', fontWeight: 800, fontSize: 20, color: N ? '#e2f0ff' : '#1a3a5c', marginBottom: 8 }}>Aucun enfant enregistré</h2>
      <p style={{ fontSize: 14, color: 'var(--t-muted)', lineHeight: 1.6, marginBottom: 24 }}>Ajoutez un profil enfant pour commencer le suivi vaccinal.</p>
      <button onClick={() => setPage('vaccins')} style={{ padding: '12px 28px', borderRadius: 12, border: 'none', background: 'linear-gradient(135deg,#1a7bca,#2196f3)', color: '#fff', fontSize: 14, fontWeight: 800, cursor: 'pointer', fontFamily: 'var(--font-main)' }}>
        Commencer le suivi
      </button>
    </div>
  </div>
...
const DashboardPage = () => {
  const { isNight: N, setPage, selectedChild, setSelectedChild, appChildren, hospitals, dataLoaded } = useApp();

  if (!dataLoaded) return <LoadingState N={N} />;

  if (appChildren.length === 0) return <EmptyState N={N} setPage={setPage} />;
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '32px 36px 40px', position: 'relative', zIndex: 1 }}>
      ...
      {child.nextVaccine?.urgent && (
        <div className="slide-in-l" style={{ background: N ? 'rgba(245,158,11,0.1)' : '#fffbeb', border: `1.5px solid ${N ? 'rgba(245,158,11,0.4)' : '#fde68a'}`, borderRadius: 16, padding: '14px 20px', marginBottom: 22, display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ fontSize: 12, color: N ? '#d97706' : '#b45309' }}>{child.nextVaccine.date} · Dans {child.nextVaccine.daysLeft} jour(s)</p>
          </div>
      ...
      <div style={{ background: N ? 'rgba(8,20,52,0.9)' : '#fff', border: `1px solid ${N ? 'rgba(100,180,255,0.1)' : 'rgba(26,123,202,0.08)'}`, borderRadius: 16, padding: '20px 22px', marginBottom: 20, boxShadow: N ? '0 4px 20px rgba(0,0,0,0.28)' : '0 4px 20px rgba(26,90,138,0.07)' }}>
        ...
        {child.vaccines.filter(v => v.status === 'upcoming').length === 0 ? (
          <p style={{ fontSize: 13, color: 'var(--t-muted)', textAlign: 'center', padding: '12px 0' }}>Aucun vaccin à venir planifié.</p>
        ) : (
          child.vaccines.filter(v => v.status === 'upcoming').map((v, i, arr) => (
            <div key={v.id || i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: i < arr.length - 1 ? `0.5px solid ${N ? 'rgba(100,180,255,0.08)' : 'rgba(26,123,202,0.07)'}` : 'none' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 11, color: 'var(--t-muted)' }}>{v.doctor ? `${v.doctor} · ` : ''}{v.hospital}</p>
                </div>
              </div>
      ...
      <div style={{ background: N ? 'rgba(8,20,52,0.9)' : '#fff', border: `1px solid ${N ? 'rgba(100,180,255,0.1)' : 'rgba(26,123,202,0.08)'}`, borderRadius: 16, padding: '20px 22px', boxShadow: N ? '0 4px 20px rgba(0,0,0,0.28)' : '0 4px 20px rgba(26,90,138,0.07)' }}>
        ...
        {hospitals.length === 0 ? (
          <p style={{ fontSize: 13, color: 'var(--t-muted)', textAlign: 'center', padding: '12px 0' }}>Aucun centre enregistré.</p>
        ) : (
          hospitals.slice(0, 3).map((h, i) => (
            <div key={h.id || i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: i < 2 ? `0.5px solid ${N ? 'rgba(100,180,255,0.08)' : 'rgba(26,123,202,0.07)'}` : 'none' }}>
              <div>
                <p style={{ fontSize: 13, fontWeight: 700, color: N ? '#e2f0ff' : '#1a3a5c', marginBottom: 2 }}>{h.name}</p>
                <p style={{ fontSize: 11, color: 'var(--t-muted)' }}>{h.distance}{h.rating ? ` · ⭐ ${h.rating}` : ''}</p>
              </div>
